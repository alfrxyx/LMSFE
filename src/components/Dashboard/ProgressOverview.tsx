...
export function ProgressOverview({ userData }: { userData: any }) {
 ...
 <p className="text-gray-500 text-[11px] font-bold uppercase tracking-widest">Belum ada aktivitas kursus.</p>
 </div>
 );
 }
 ...
 const masterCompletedIds = Array.from(new Set([...completedFromProfile, ...completedFromCourses]));

 const completedCount = levels.filter((l: any) => masterCompletedIds.includes(Number(l.id))).length;
 ...
 <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-1">Struktur Kurikulum PJKR</p>
 </div>
 ...
 isDone ? 'bg-green-100 shadow-inner' : 'bg-blue-600 shadow-lg shadow-blue-100'
 }`}>
 {isDone ? (
 ...
 <div>
 <p className={`text-sm font-black tracking-tight ${isDone ? 'text-gray-500 line-through' : 'text-gray-900'}`}>
 {level.title || level.name || `Pertemuan ${index + 1}`}
 </p>
 <div className="flex items-center gap-2 mt-0.5">
 <span className="text-[9px] font-bold text-gray-400 uppercase tracking-widest">PERTEMUAN {index + 1}</span>
 </div>
 </div>
 </div>
 ...
 <p className="text-[10px] font-black text-gray-300 uppercase tracking-[0.3em]">Materi Kosong</p>
 </div>
 )}
 </div>
 ...
 <div>
 <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Total Capaian</p>
 <p className="text-xl font-black text-gray-900 tracking-tighter">
 {levels.length > 0 ? Math.round((completedCount / levels.length) * 100) : 0}%
 </p>
 </div>
